...
![Diagram of the tundra ramet model of light competition.](/Users/ruby/Projects/shrub_model/diagrams/two_ramets_compete_for_light.png) 
...
The first version of this model includes only competition for light, assuming light extinction following Beer's law. 
...
Each species occupies one height $H_i$ in the canopy. The light that reaches the canopy height of species $i$ is set by the number of ramets of species taller ($j < i$) than species $i$. Ramets of species $i$ share the light that reaches their canopy equally. 
...
This means that the light must be AT LEAST this level in order for species of a given height to establish. 

```{r}
# full parameter list: species + environment parameters 
p <- list(v = 1, # leaf density: leaf area per unit canopy area
             b = 1, # fecundity
             m = 1, # mortality
             a = 10, # photosynthetic carbon gain
             r = 2, # leaf respiration
             c_l = 0.5, # construction costs of leaves
             L_above = 100 # light level above the canopy
             )

with(as.list(p), {
  # establishment criterion
  H_0 <- (log(2)*v/(b*m)*(a-r-c_l))^(1/5)
  ...
  L = (1/a)*(H^5*b*m/(log(2)*v)+r+c_l)

  data <- tibble(height = H, light = L)
  ggplot(data, aes(x=height, y = light)) +
    geom_line() +
    geom_hline(yintercept = (r + c_l)/a, linetype = 2, color = "orange") +
...
# calculate community
eq_state <- light_extinction(S=50, # of species
                             k=0.5, # light extinction
                             u_min = r/(a*k)) %>% # phys. limit of light
...
# calculate community
eq_state <- light_extinction(S=30, # of species
                             k=0.5, # light extinction
...
# calculate community
eq_state_w <- light_extinction_warmed(S=30, # of species
                             k=0.5, # light extinction
                             u_min = u_min) %>% # phys. limit of light
  mutate(feasible = if_else(x == 0, FALSE, TRUE))
...
# calculate community
eq_state <- light_extinction(S=40, # of species
                             k=0.5, # light extinction
                             u_min = u_min) %>% # phys. limit of light
  mutate(feasible = if_else(x == 0, FALSE, TRUE))
